...
(function () {
  ...
  function explainCost(costStr, tier) {
    ...
    const m = costStr.match(/₹?\s*([\d,]+(?:\.\d+)?)\s*(?:[–-]\s*[\d,]+(?:\.\d+)?)?\s*Cr/i);
    if (!m) return costStr;
    const cr = parseFloat(m[1].replace(/,/g, ''));
    const recurring = /\/yr|annually|annual/i.test(costStr);
    const oneTime = /one[-\s]?time/i.test(costStr);
    const TN_TAX_REVENUE_CR = 200000; // ~₹2 lakh crore TN own-tax revenue
    const pct = (cr / TN_TAX_REVENUE_CR * 100);

    switch (tier) {
      case 'K': {
        // Convert to "if you collected ₹100 in pocket money, this scheme would eat ₹X"
        const perHundred = Math.round(pct);
        if (perHundred >= 1) {
      ...
      case 'E': {
        const debt_pct = (cr / 1071000 * 100); // TN debt ~₹10.71 L Cr
        const capex_pct = (cr / 60000 * 100);  // TN annual capex ~₹60K Cr
  ...
  function renderSchemeForTier(scheme, tierId) {
    const t = tierId || 'V';
    const verdictTxt = (VERDICT_HUMAN[scheme.verdict] || VERDICT_HUMAN.U)[t];
    const starTxt = (STAR_HUMAN[scheme.stars] || STAR_HUMAN[0])[t];
    const partyTxt = (PARTY_HUMAN[scheme.party] || { [t]: scheme.party })[t];
    const costTxt = explainCost(scheme.cost, t);
    ...
    return {
      ...
      pros: scheme.pros.map(p => simplifyLine(p, t)),
      cons: scheme.cons.map(c => simplifyLine(c, t)),
  ...
  function kidify(text) {
    if (!text) return '';
    return text
      .replace(/₹\s*([\d,]+)\s*Cr/g, (m, n) => `${n} crore rupees`)
  ...
  function framedContext(s, t) {
    ...
    if (t === 'V') return ctx;
    if (t === 'W') return 'Comparative evidence: ' + ctx;
    if (t === 'E') return 'Empirical comparators: ' + ctx;
  }

  function framedRelevance(s, t) {
    const rel = s.tnRelevance || '';
    if (!rel) return '';
    ...
    if (t === 'V') return rel;
    if (t === 'W') return 'TN-specific context: ' + rel;
    if (t === 'E') return 'TN baseline & sensitivity: ' + rel;
  }
  ...
  window.AudienceTiers = {
    ...
    glossary(key, tier) {
      const t = tier || window.AudienceTiers.current();
      const entry = UI_GLOSSARY[key];
